...
class BoalGame(Board):
    ...
    def circle_drow(self, y, color, screen, coord, i):
        if color == 'Blue':
            if i is not None:
                y_pos = 120  # Начало падения
                v = 150  # Начальная скорость
                clock = pygame.time.Clock()  # объект,который поможет отслеживать время
                while True:
                    ...
                    y_pos += v * clock.tick() / 1000 # Расчет скорости падения
                    v *= 1.005 # Увеличивание скорости
                    pygame.display.flip()
                    if y_pos >= y - 50 * (i[1]): # Завершение падения
                        break
            else:
                y_pos = 120
                v = 150
